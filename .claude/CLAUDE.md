...
- 长任务使用 `TaskCreate` / `TaskUpdate` 维护少量用户可见的顶层待办,用户可按 `Ctrl+T` 显示或隐藏待办列表,也可直接要求列出全部待办;`/tasks` 仅用于后台 shell 与 subagent。子代理内部步骤不得创建为顶层待办。每完成一个顶层待办,主动播报 `进度 N/M`、结果和下一步;`.superpowers/sdd/progress.md` 仅用于会话恢复,不替代结构化待办和里程碑播报。
