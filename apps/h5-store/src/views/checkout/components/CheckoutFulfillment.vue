...
function updateMode(event: Event): void {
  emit(
    'update:fulfillmentType',
    (event.target as HTMLInputElement).value as FulfillmentType,
  );
}
...
<template>
  <fieldset class="store-form-card checkout__field">
    <legend class="store-form-card__heading">
  ...
  <section class="store-form-card checkout__fulfillment-detail">
    ...
    <label
      v-if="fulfillmentType === FulfillmentType.PICKUP"
        ...
        <li v-for="address in addresses" :key="address.id">
          ...
          >
            ...
            <span
              ...
              <span v-if="address.isDefault" class="checkout__badge">默认</span
              ><small
                >{{ address.province }} {{ address.city }}
                {{ address.district }} {{ address.detail }}</small
              ></span
            >
          </label>
        </li>
...
.store-form-card__heading strong {
  margin: 0;
  font-size: 15px;
  font-weight: 700;
}
.checkout__field,
.checkout__addresses-field {
  min-width: 0;
}
.checkout__addresses-field {
  margin: 0;
...
.checkout__addresses-field > legend,
.checkout__control > span {
  color: var(--mall-text-muted);
  font-size: 12px;
}
.checkout__radio-grid {
  display: grid;
...
.checkout__radio--address {
  align-items: flex-start;
}
.checkout__radio--address > span {
  min-width: 0;
  line-height: 1.5;
}
...
.checkout__badge {
  ...
  margin-left: var(--mall-space-1);
  padding: 1px var(--mall-space-2);
  border-radius: 999px;
  background: var(--mall-surface-soft);
  color: var(--mall-primary-strong);
  font-size: 11px;
