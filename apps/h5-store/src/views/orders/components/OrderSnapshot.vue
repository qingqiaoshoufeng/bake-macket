...
<template>
  <header class="order-detail__hero">
    ...
    >
    <p class="order-detail__eyebrow">订单编号</p>
    <h1>{{ order.orderNo }}</h1>
    <p>下单时间:{{ formatOrderDate(order.createdAt) }}</p>
  </header>
  ...
  <section class="order-detail__section order-detail__section--items">
    ...
    <ul class="order-detail__items">
      <li v-for="item in order.items" :key="item.id">
        <div class="order-detail__item-name">{{ item.productName }}</div>
        <div class="order-detail__item-sku">
          {{ item.skuName }}
          <span v-if="formatSkuAttributes(item.skuAttributes)"
            >({{ formatSkuAttributes(item.skuAttributes) }})</span
          >
        </div>
...
.order-detail__eyebrow {
  margin-top: var(--mall-space-4) !important;
  color: var(--mall-primary-strong) !important;
...
.order-detail__items li {
  padding-bottom: var(--mall-space-3);
