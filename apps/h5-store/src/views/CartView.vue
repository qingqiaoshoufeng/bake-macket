...
<template>
  <main class="cart-view">
    <header>
      <small>READY TO BAKE</small>
      <h1>{{ CART_COPY.title }}</h1>
      <p>共 {{ itemCount }} 件可结算商品</p>
    </header>
    <p v-if="cart.loading && !cart.items.length" class="state-copy">
      正在整理购物车…
    </p>
    <section v-else-if="!cart.items.length" class="empty-card">
      <span>🥐</span>
      <p>{{ CART_COPY.empty }}</p>
      <button type="button" @click="router.push('/')">去首页挑选</button>
    </section>
    <ul v-else class="cart-list">
      <li
        v-for="item in cart.items"
        :key="item.id"
        :class="['cart-row', !item.available && 'is-invalid']"
      >
        ...
        <div class="cart-row__body">
          <div class="cart-row__title">
            ...
            <em v-if="!item.available">{{ CART_COPY.invalid }}</em>
          </div>
          <p>{{ item.sku.name }}</p>
...
.cart-row {
  ...
  border-radius: 20px;
  background: #fff;
  box-shadow: 0 8px 24px rgba(73, 62, 49, 0.07);
}
.cart-row.is-invalid {
  opacity: 0.62;
...
.checkout-bar {
  ...
  background: #334b39;
  color: #fff;
  box-shadow: 0 14px 32px rgba(43, 57, 46, 0.25);
}
.checkout-bar div {
  display: flex;
  flex-direction: column;
