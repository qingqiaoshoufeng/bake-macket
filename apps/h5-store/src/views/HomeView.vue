  ...
  <main class="home-shell">
    ...
    <section class="section-block">
      <header class="section-head">
        <div>
          <small>店内常被带走</small>
          <h2>{{ CATALOG_COPY.popularTitle }}</h2>
        </div>
      </header>
      <p
      ...
      <p v-else-if="!catalog.products.value.length" class="state-copy">
        今天的烘焙还在准备中。
      </p>
...
.hero {
  ...
  background: linear-gradient(140deg, #dcebd8, #f7ddbd);
  box-shadow: 0 18px 42px rgba(91, 105, 70, 0.13);
}
.hero::after {
  content: 'BAKE';
  position: absolute;
...
.banner-reel__frame {
  ...
  padding: 0;
  background: #e7d9c7;
  box-shadow: 0 12px 26px rgba(79, 65, 49, 0.11);
}
.banner-reel img {
  width: 100%;
  height: 100%;
...
.category-strip button {
  ...
  align-items: center;
  gap: 7px;
  border: 1px solid rgba(125, 167, 125, 0.2);
  border-radius: 999px;
