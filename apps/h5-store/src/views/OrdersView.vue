...
<template>
  <StorePage with-tabbar class="orders">
    <StorePageHeader
      title="我的订单"
      eyebrow="ORDER NOTES"
      description="显示最近创建的订单,包括新订单、处理中、已完成和已取消。"
    />
    <StoreStatePanel
      v-if="orders.loading.value && !orders.data.items.value.length"
      state="loading"
      title="正在翻阅订单记录"
      description="稍等一下，很快就好。"
    />
    <StoreStatePanel
      v-else-if="!orders.data.items.value.length"
      state="empty"
      title="暂无订单,先去下单吧。"
      description="挑选喜欢的烘焙后，订单进度会显示在这里。"
      ><template #action
        ><button class="orders__browse" type="button" @click="navigate('/')">
          去首页挑选
        </button></template
      ></StoreStatePanel
    >
    <ul v-else class="orders__list">
      <OrderCard
        v-for="order in orders.data.items.value"
        :key="order.id"
        :order="order"
        @open="openDetail"
      />
