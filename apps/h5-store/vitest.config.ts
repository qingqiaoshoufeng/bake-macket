...
/**
 * Vitest configuration for the storefront.
 *
 * Pinia stores interact with `window.localStorage` and the shared
 * `ApiClient` reaches for `fetch`, so the default `node` environment isn't
 * enough — we switch every test to `jsdom`. The SFC/component tests added
 * in later tasks continue to run under the same browser-shaped globals.
 */
