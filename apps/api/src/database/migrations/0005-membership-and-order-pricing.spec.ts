...
describe('MembershipAndOrderPricing migration', () => {
  it('creates the seven membership tables with project schema conventions', async () => {
    ...
    for (const table of MEMBERSHIP_TABLES) {
      ...
      expect(createSql).toContain('`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT');
  ...
  it('adds immutable pricing snapshots without changing existing order values', async () => {
    ...
    expect(sql).toContain('`membership_discount_basis_points` INT UNSIGNED NULL');
    expect(sql).toContain('`pricing_version` INT UNSIGNED');
    expect(sql).toContain('UPDATE `orders` SET `payable_total_cents` = `goods_total_cents`');
    expect(sql).toContain('`payable_total_cents` = `goods_total_cents` - `membership_discount_cents` - `credit_applied_cents`');
  ...
  it('upgrades legacy idempotency rows before replacing the unique key', async () => {
    ...
    expect(upgradeSql).toContain("`operation` VARCHAR(64) NOT NULL DEFAULT 'PRODUCT_ORDER_CREATE'");
    expect(upgradeSql).toContain('`request_hash` CHAR(64) NULL');
    expect(upgradeSql).toContain("`status` ENUM('IN_PROGRESS','COMPLETED','FAILED') NOT NULL DEFAULT 'COMPLETED'");
    ...
    expect(upgradeSql).toContain("UPDATE `idempotency_records` SET `resource_type` = 'ORDER', `resource_id` = CAST(`order_id` AS CHAR)");
    ...
    const addIndex = statements.findIndex((sql) =>
      sql.includes('UNIQUE INDEX `uniq_idempotency_user_operation_key` (`user_id`, `operation`, `key`)'),
    );
  ...
  it('drops membership and pricing schema in dependency-safe order', async () => {
    ...
    const statements = query.mock.calls.map(([sql]) => String(sql));
    expect(statements[0]).toContain('idempotency_records');
    expect(statements.join('\n')).toContain('DROP TABLE IF EXISTS `member_credit_allocations`');
    expect(statements.join('\n')).toContain('DROP TABLE IF EXISTS `membership_levels`');
    expect(statements.at(-1)).toContain('idempotency_records');
  });
});
