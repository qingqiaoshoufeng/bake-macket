...
describe('DATABASE_MIGRATIONS 注册表完整性', () => {
  it('统一注册十一项迁移，并以云打印机迁移作为当前尾项', () => {
    expect(DATABASE_MIGRATIONS).toHaveLength(11);
  ...
  it('按 timestamp 顺序注册迁移', () => {
    const timestamps = DATABASE_MIGRATIONS.map((migration) =>
      timestampOf(migration.name),
    );

    expect(
      timestamps.every((timestamp, index) =>
        index === 0 ? true : timestamp > timestamps[index - 1],
      ),
    ).toBe(true);
  });
});
