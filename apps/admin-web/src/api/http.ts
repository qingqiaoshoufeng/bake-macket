 ...
 *
 * Shared shape with the storefront's client — the wrapper is deliberately
 * kept audience-agnostic so the auth store can register an unauthorized
 * handler that drops a session without leaking H5 ↔ admin state.
 */
...
/**
 * Shared `fetch` wrapper for the merchant admin SPA. Behaviour mirrors the
 * storefront client:
 *
 * - prefixes every request with the configurable `/api/v1` base so callers
 *   pass the literal NestJS route (`/admin/auth/login`, …);
 * - serialises JSON bodies and sets the matching `content-type`;
 * - forwards the caller-supplied (or admin store-supplied) bearer token;
 * - converts failures to {@link ApiClientError} so components can `instanceof`
 *   -check without parsing `response.json()` themselves;
 * - on a `401`, invokes the registered unauthorized handler so the admin
 *   auth store can clear its session and the router can bounce the merchant
 *   back to `/login?redirect=…` before any further requests fire.
 *
 * The merchant-token wiring deliberately lives in the admin store — keeping
 * the audience split prevents the customer `bake_user_token` from being
 * forwarded to `/admin/*` and vice versa.
 */
...
/**
 * Module-level default client. Components import `apiClient` so the admin
 * auth store can register its unauthorized handler exactly once at app boot.
 */
