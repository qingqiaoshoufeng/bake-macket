...
<template>
  <div class="admin-horizontal-scroll">
    <ElTable
      v-loading="loading"
      :data="[...levels]"
      row-key="id"
    ...
    >
      <ElTableColumn
        :label="levelColumn.label"
        :min-width="levelColumn.minWidth"
      >
      ...
      <ElTableColumn
        ...
        :min-width="priceColumn.minWidth"
      >
        <template #default="scope">
          <div class="membership-card-table__money">
            <strong>{{ formatPriceCents(rowOf(scope).priceCents) }}</strong>
            <span
              >赠 {{ formatPriceCents(rowOf(scope).grantCreditCents) }}</span
            >
          </div>
        </template>
      </ElTableColumn>
      <ElTableColumn
        :label="discountColumn.label"
